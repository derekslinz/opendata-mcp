...
- ``OPENDATA_MCP_DOMAINS``
    Comma-separated list of domain names (see ``meta_data_mcp.registry.DOMAINS``)
...
from meta_data_mcp.registry import REGISTRY, find_providers
...
# Module names within ``meta_data_mcp.providers`` that should never be treated
...
async def main(
    ...
    for provider_id in selected_ids:
        if provider_id in _SKIP_MODULES:
            continue
        module_path = f"meta_data_mcp.providers.{provider_id}"
        try:
