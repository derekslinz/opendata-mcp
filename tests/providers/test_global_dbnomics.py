...
async def test_dbnomics_search_success():
    with patch("httpx.get") as mock_get:
        mock_get.return_value.json.return_value = {"datasets": {"docs": [{"code": "WEO"}]}}
...
async def test_dbnomics_list_providers_success():
    with patch("httpx.get") as mock_get:
        mock_get.return_value.json.return_value = {"providers": {"docs": [{"code": "IMF"}]}}
        mock_get.return_value.raise_for_status = Mock()
...
async def test_dbnomics_series_success():
    with patch("httpx.get") as mock_get:
        mock_get.return_value.json.return_value = {"series": {"docs": [{"code": "NGDP"}]}}
        mock_get.return_value.raise_for_status = Mock()
