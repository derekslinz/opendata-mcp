...
async def test_awc_metar_success():
    with patch("httpx.get") as mock_get:
        mock_get.return_value.json.return_value = [{"icaoId": "KJFK", "rawOb": "KJFK 111851Z ..."}]
...
async def test_awc_taf_success():
    with patch("httpx.get") as mock_get:
        mock_get.return_value.json.return_value = [{"icaoId": "KJFK", "rawTaf": "KJFK 111720Z ..."}]
        mock_get.return_value.raise_for_status = Mock()
...
async def test_awc_station_success():
    with patch("httpx.get") as mock_get:
        mock_get.return_value.json.return_value = [{"icaoId": "KJFK", "name": "NEW YORK/JOHN F KENNEDY INTL"}]
        mock_get.return_value.raise_for_status = Mock()
